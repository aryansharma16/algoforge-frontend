      ...
      <div className="rounded-lg border border-slate-700 bg-slate-950 p-2">
        <div className="mb-2 flex flex-wrap gap-1.5">
          {value.map((tag) => (
            <span
              key={tag}
              className="inline-flex items-center gap-1 rounded-md bg-violet-950/60 px-2 py-0.5 text-xs text-violet-200 ring-1 ring-violet-500/25"
            >
              {tag}
              <button
                type="button"
        ...
        <div className="relative">
          <input
            ...
            className="w-full border-0 bg-transparent py-1 text-sm text-slate-100 outline-none placeholder:text-slate-600"
          />
          {open && rows.length > 0 && value.length < max && (
            <ul className="scrollbar-themed absolute z-20 mt-1 max-h-48 w-full overflow-y-auto overflow-x-hidden rounded-lg border border-slate-600 bg-slate-900 py-1 shadow-lg">
              {rows.map((row, i) => (
                <li
                  ...
                  className={`cursor-pointer px-3 py-2 text-sm ${
                    row.kind === 'add' ? 'text-violet-300' : 'text-slate-200'
                  } hover:bg-slate-800`}
                >
