...
import JourneyCardSkeleton from '../components/skeletons/JourneyCardSkeleton'
  ...
  return (
    <div>
      ...
      {list.length === 0 ? (
        ...
        <>
          {/* Toolbar: on mobile only search + toggle; filters/sort/layout behind toggle. On md+ all visible. */}
          <div className="mb-5 flex flex-col gap-3 rounded-xl border border-slate-200 bg-slate-50/80 px-3 py-3 dark:border-slate-700 dark:bg-slate-800/40 md:flex-row md:flex-wrap md:items-center md:gap-3">
            ...
            <div className="flex min-w-0 flex-1 items-center gap-2">
              ...
              <button
                ...
                className="flex shrink-0 items-center gap-1.5 rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-medium text-slate-700 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200 md:hidden"
                aria-expanded={filtersOpenMobile}
                aria-controls="journeys-filters-panel"
                id="journeys-filters-toggle"
              >
                Filters & sort
                <svg
                  className={`h-4 w-4 transition-transform ${filtersOpenMobile ? 'rotate-180' : ''}`}
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
            ...
            {/* Filters / layout / sort: collapsible on mobile, always visible on md+ */}
            <div
              id="journeys-filters-panel"
              role="region"
              aria-labelledby="journeys-filters-toggle"
              className={`grid transition-[grid-template-rows] duration-200 md:contents ${
                filtersOpenMobile ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'
              }`}
            >
              <div className="flex min-h-0 flex-wrap items-center gap-2 overflow-hidden md:overflow-visible md:flex-1 md:flex-initial">
                <div className="flex w-full flex-wrap items-center gap-2 pt-1 md:w-auto md:pt-0">
                ...
                <div className="flex w-full flex-wrap items-center gap-2 border-t border-slate-200 pt-2 dark:border-slate-600 md:w-auto md:border-t-0 md:pt-0 md:pl-2 md:border-l md:border-slate-200 md:dark:border-slate-600">
